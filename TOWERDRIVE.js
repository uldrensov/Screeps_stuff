...
assess:         for (let i=0; i<enemy.length; i++){
            ...
            var injured_units = nexi[k].room.find(FIND_MY_CREEPS, {
                filter: creep => {
                    return creep.hits < creep.hitsMax;
                }
            });
            var repairStructs = nexi[k].room.find(FIND_STRUCTURES, {
                filter: structure => {
            ...
            var repairRamparts = nexi[k].room.find(FIND_STRUCTURES, {
                filter: (structure) => {
            ...
            if (threat){
                ...
                if (Game.getObjectById(Memory.turretTarget_id[k]).owner.username != 'Invader')
                    Game.notify(target_enemy.owner.username + ' DETECTED IN ROOM #' + home_index,30);
            }
            //perform other tasks only if energy can be spared, and construction mode is disabled
                ...
                //unload: structures
                else if (repairRamparts.length || repairStructs.length){
