    ...
    run: function(room_num, o_type, o_amt, spec, dir, autokill){
        ...
        switch (spec){
            ...
            case 'NUK':
                ...
                break;
            case 'LAB':
                    return 'TERMINALTRANSFER:: FEATURE COMING SOON...';
                break;
            default:
        ...
        if (!treasurer){
            ...
            let spawnResult = openNexus.spawnCreep(SD.treas_body, 'Treasurer-' + Game.time % SD.time_offset,
                {memory: {role: 'treasurer', order_type:o_type, order_amt:o_amt, dir:dir, spec_dest:spec, task_progress:0, autokill:autokill}});
            if (spawnResult == OK)
                console.log('TERMINALTRANSFER:: Room #' + room_num + ': Treasurer-' + Game.time % SD.time_offset + ' spawning.');
            else
                return 'TERMINALTRANSFER:: NEXUS ERROR: ' + spawnResult;
        }
        ...
        return 'TERMINALTRANSFER:: ISSUING COMMAND: ROOM #' + room_num + ' ' + dir_toString + ' ' + o_amt + ' [' + o_type + '] ' + to_fro + ' ' + dest_toString;
    }
};